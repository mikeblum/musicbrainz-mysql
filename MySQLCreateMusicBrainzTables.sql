...
CREATE TABLE IF NOT EXISTS area ( -- replicate (verbose)
    ...
    ended               VARCHAR(1) NOT NULL DEFAULT FALSE
      CHECK (
        (
...
CREATE TABLE IF NOT EXISTS isrc ( -- replicate (verbose)
    ...
    isrc                CHAR(12) NOT NULL CHECK (isrc REGEXP '^[A-Z]{2}[A-Z0-9]{3}[0-9]{7}$'),
    source              SMALLINT,
    edits_pending       INTEGER NOT NULL DEFAULT 0 CHECK (edits_pending >= 0),
    created             TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS iswc ( -- replicate (verbose)
    ...
    iswc CHARACTER(15) CHECK (iswc REGEXP '^T-?\\d{3}.?\\d{3}.?\\d{3}[-.]?\\d$'),
    source SMALLINT,
...
CREATE TABLE IF NOT EXISTS link ( -- replicate
    id                  SERIAL,
